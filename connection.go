...
func DialClient(addresses []string) (*client, *grpc.ClientConn, error) {
	...
	for _, addr := range addresses {
		...
		resp, err := grpcClient.GetLeader(context.Background(), &pb.Empty)
		if err != nil {
			conn.Close()
			lastErr = err
			continue
		...
		conn.Close()
		leaderConn, err := grpc.NewClient(addr, grpc.WithTransportCredentials(insecure.NewCredentials()))
		if err != nil {
			lastErr = err
			continue
		}
		leaderClient := pb.NewFileServiceClient(leaderConn)
		customClient := NewClient(leaderClient)
